...
async function run(): Promise<void> {
  try {
    ...
    if (version) {
      windowsInstallScript = `powershell -ex AllSigned -c "Invoke-RestMethod 'https://aka.ms/install-azd.ps1' -OutFile 'install-azd.ps1'; sudo ./install-azd.ps1 -Version '${version}'"`
